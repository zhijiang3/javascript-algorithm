import { default as DoubleLinkedList, DoubleLinkedListNode } from "data-structure/DoubleLinkedList";
...
describe("空值", () => {
  ...
  test("在链表中插入节点", () => {
    ...
    list.insertAfter(new DoubleLinkedListNode(1), 1);
    expect(list.head).toBeNull();
    expect(list.tail).toBeNull();
  });
});
...
describe("在尾部追加链表", () => {
  ...
  test("连接两个链表，并且指向正确", () => {
    const appendList = new DoubleLinkedList().append(3).append(4);
...
describe("在头部追加链表", () => {
  ...
  test("连接两个链表，并且指向正确", () => {
    const prependList = new DoubleLinkedList().prepend(2).prepend(1);
...
describe("在链表中添加节点", () => {
  const list = new DoubleLinkedList().append(3).prepend(1);
  ...
  test("在链表中间添加一个链表，并且指向正确", () => {
    const insertAfterList = new DoubleLinkedList().append(5).append(6);
    list.insertAfter(
  ...
  test("在链表尾部添加", () => {
    const insertAfterList = new DoubleLinkedList().prepend(8).prepend(7);
